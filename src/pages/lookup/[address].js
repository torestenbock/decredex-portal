...
const StudentLookup = (props) => {
  ...
  const contract = new ethers.Contract(context.decredex.networks["920717"].address, context.decredex.abi, context.provider)
  const { address } = router.query

  React.useEffect(() => {
    contract.queryStudent(address).then((result) => {
      setStudent({
        name: result[1],
        ...
        <div className={classes.content}>
          ...
          <div className={classes.contentHeader}>
            <Typography variant="h6">
              {t("common:showing-for")} {student?.name}
            </Typography>
